...
describe('Redux Deep Diff: reducer', function() {
  ...
  it('should have an initial empty history', function() {
    expect(this.state).to.have.property('diff')
      .that.has.all.keys('prev', 'last', 'next');
    expect(this.state.diff.prev).to.be.an('array').with.lengthOf(0);
    expect(this.state.diff.last).to.be.an('array').with.lengthOf(0);
  ...
  it('should show the last diff when the state changes', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(6);
    expect(this.state.diff.last[0]).to.beDiff('E', ['string'], 'hello', 'world');
    expect(this.state.diff.last[1]).to.beDiff('E', ['number'], 1000, 500);
    expect(this.state.diff.last[2]).to.beDiff('E', ['boolean'], true, false);
    expect(this.state.diff.last[3]).to.beDiff('A', ['array'], 3, { kind: 'N', rhs: 'd' });
    expect(this.state.diff.last[4]).to.beDiff('D', ['object', 'c'], 2);
    expect(this.state.diff.last[5]).to.beDiff('N', ['object', 'C'], undefined, 2);
  });
  ...
  it('should show previous diffs when additional state changes are made', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(1);
    expect(this.state.diff.last[0]).to.beDiff('E', ['boolean'], true, false);

    expect(this.state.diff.prev).to.have.lengthOf(2);
    expect(this.state.diff.prev[0]).to.have.lengthOf(1);
    expect(this.state.diff.prev[0][0]).to.beDiff('E', ['number'], 1000, 9999);
    expect(this.state.diff.prev[1]).to.have.lengthOf(1);
    expect(this.state.diff.prev[1][0]).to.beDiff('E', ['string'], 'hello', 'cat');
  });
  ...
  it('should undo the last diff by reverting changes', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(0);
  ...
  it('should redo the next diff by applying changes', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(1);
    expect(this.state.diff.last[0]).to.beDiff('E', ['number'], 1000, 1);
  });
  ...
  it('should redo all undone changes up to a specified index', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(1);
    expect(this.state.diff.last[0]).to.beDiff('E', ['string'], 'hello', '$#@%&*!');
    expect(this.state.diff.next).to.have.lengthOf(1);
    expect(this.state.diff.next[0]).to.have.lengthOf(1);
  ...
  it('should clear the next diffs when a new diff occurs', function() {
    ...
    expect(this.state.diff.last).to.have.lengthOf(1);
    expect(this.state.diff.last[0]).to.beDiff('E', ['string'], 'hello', 'clarice');
    expect(this.state.diff.next).to.have.lengthOf(0);
  });
